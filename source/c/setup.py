...
- 🔄 Compiles `.pyx` (Cython) → `.c` → shared object (`.so` / `.pyd`)
...
ext = Extension(
    name='solution',              # 📦 Name of compiled module (used in Python import)
    sources=['solution.pyx'],     # 📄 Cython source file
    language='c'                  # ⚙️ Compile using C (not C++)
)
...
setup(
    ext_modules=cythonize(
        ext,
        language_level=3          # 🐍 Ensure Python 3 syntax compatibility
    )
)